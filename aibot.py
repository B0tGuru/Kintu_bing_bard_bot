...
#print(tele_token)
#"5705794499:AAEIIrRfXW_gAk2PV3i-tjhRwHX_xNhefBU"#str()
...
async def main(web_url):
    ...
    logger.info(f"init ok {web_url}")
    logging.error(f"init ok {web_url}")
    ...
    webhook_url = web_url+'/tbard/webhook'
    logging.error(f"settingup bot {webhook_url}")
    abot = tapp.bot#
    print(abot)
    print(f"\nusing webhook {webhook_url}")
    ...
    #        print("all well")
    #        break
    print("web update finished")
    #tapp.bot = bot
    webin = await abot.getWebhookInfo()
    #tapp.bot = bot
